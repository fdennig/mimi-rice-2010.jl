...
function construct_nice(inputs::NICE_inputs)
    ...
    damage_dist = quintile_dist(inputs.xi, income_dist)
    abatement_dist = quintile_dist(inputs.omega, income_dist)
    ...
    pop_data = 0.2 .* reshape(repmat(rice_population,1,5), inputs.nsteps, 12, 5)
    ...
    setparameter(m, :grosseconomy, :dk, inputs.cap_dep)

    setparameter(m, :emissions, :MIU, inputs.abate)
    ...
    setparameter(m, :nice_welfare, :rho, inputs.rho)
    setparameter(m, :nice_welfare, :eta, inputs.eta)

    # Set new savings rate (base NICE value differs from RICE savings rate)
    setparameter(m, :neteconomy, :S, (ones(inputs.nsteps,12) .* 0.2585))
...
function construct_nice_objective(inputs::NICE_inputs)
    ...
    function nice_objective(tax::Array{Float64,1})
        ...
        abatement_level, tax = mu_from_tax(tax, rice_backstop, 2.8)
